...
class BeehiveModel:
    ...
    def create(db, apiary_id, hive_number, **kwargs):
        ...
        query = f"INSERT INTO hives ({', '.join(fields)}) VALUES ({', '.join(placeholders)})"
        cursor = BeehiveModel._execute_update(db, query, values)
        return cursor.lastrowid
    
    @staticmethod
    def get_by_id(db, hive_id):
        result = BeehiveModel._execute_query(db, 'SELECT * FROM hives WHERE id = ?', (hive_id,))
        return dict(result[0]) if result else None
    
    @staticmethod
    def get_by_apiary(db, apiary_id):
        result = BeehiveModel._execute_query(db, 'SELECT * FROM hives WHERE apiary_id = ? ORDER BY hive_number', (apiary_id,))
    ...
    def update(db, hive_id, **kwargs):
        ...
        query = f"UPDATE hives SET {', '.join(fields)}, updated_at = CURRENT_TIMESTAMP WHERE id = ?"
        BeehiveModel._execute_update(db, query, params)
    
    @staticmethod
    def delete(db, hive_id):
        BeehiveModel._execute_update(db, 'DELETE FROM hives WHERE id = ?', (hive_id,))
    
    @staticmethod
    def get_hive_number(db, apiary_id, hive_number):
        result = BeehiveModel._execute_query(
            db, 
            'SELECT * FROM hives WHERE apiary_id = ? AND hive_number = ?',
